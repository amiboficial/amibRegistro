...
app.HRevocacionVM = Backbone.Model.extend ({
	defaults: {
		grailsId: -1,
		numeroEscritura: -1,
		nombreCompletoNotario: "",
		fechaRevocacion: "",
		fechaRevocacionUnixEpoch: -1,
...
app.HRevocacionVMCollection = Backbone.Collection.extend({
	model: app.HRevocacionVM,
	
	_offset: 0,
	_max: 10,
	
	_sort: 'asc',
	_order: 'fechaRevocacionUnixEpoch',
	...
	comparator: function(itemA, itemB){
		...
		else if(this._sort == "fechaRevocacion" || this._sort == "fechaRevocacionUnixEpoch"){
			if(this._order == "desc"){
				if(parseInt(itemA.get('fechaRevocacionUnixEpoch')) == parseInt(itemB.get('fechaRevocacionUnixEpoch')))
					return 0;
				else if(parseInt(itemA.get('fechaRevocacionUnixEpoch')) > parseInt(itemB.get('fechaRevocacionUnixEpoch')))
					return -1;
				else
					return 1;
			}
...
app.HRevocacionVMCollectionView = Backbone.View.extend({ 
	el: '#divHistoricoRevocacionSustentante',
	template: _.template( $('#hrevocsTemplate').html() ),
	templateElement: _.template( $('#hrevocTemplate').html() ),
	
	_revocacionUrl: '',
	
	initialize: function(options){
		this.collection = new app.HRevocacionVMCollection(options.elementsArray);
		this._revocacionUrl = options.revocacionUrl;
	...
	irAPoder: function(ev){
		location.assign(this._revocacionUrl + '/' + this.$(ev.currentTarget).data("id"));
	}
