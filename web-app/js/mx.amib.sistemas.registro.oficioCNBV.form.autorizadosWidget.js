...
app.AutorizadosView =  Backbone.View.extend({
	...
	initialize: function( initialData ){
		...
		this.listenTo( this.collection, 'add', this.renderAutorizado );
	...
	buscarPorMatricula: function(e){
		...
		else
		{
			...
			if(yaExisteMatricula == true){
				...
				//placeholders de camino feliz
				this._newMatricula = 666;
				this._newNombreCompleto = "LUCIFER LUCIFER LUCIFER";
				
				this.state = app.AUT_STATUS_LISTO_MAT;
				this.error = app.AUT_NO_ERR;
				this.render();
				
				//aqui se hace una llamada ajax
				//de ser satisfactorio, entonces setea un estado "listo con matricula" y renderea
				//si no, manda un estado "listo", setea el error y renderea
			}
		}
	},
});
